...
setup_test_users() {
    ...
    local users_file="$CONFIG_DIR/mods-config/files/users"
...
validate_config() {
    ...
    if freeradius -C -d "$CONFIG_DIR" >/dev/null 2>&1; then
        log "✓ FreeRADIUS configuration is valid"
    else
        error "FreeRADIUS configuration validation failed"
        info "Running detailed configuration check..."
        freeradius -C -d "$CONFIG_DIR"
        return 1
    fi
}
...
setup_signal_handlers() {
    ...
    shutdown_handler() {
        ...
        if pgrep freeradius >/dev/null; then
            ...
            pkill -TERM freeradius || true
            sleep 5
            pkill -KILL freeradius >/dev/null 2>&1 || true
        fi
